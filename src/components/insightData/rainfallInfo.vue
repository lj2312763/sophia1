<template>
  <div class="SunshineInfo_container">
     <div class="china_map_container" id="chinaMap"></div>
      <div class="line_title">2018年{{address}}降雨量趋势图</div>
  ...
  methods:{
    initChinaMap(){
        ...
        this.$axios({
            ...
            if(res.status == 200){
                var rainFallData = res.data.data;
                var geoCoordMap = {};
                var data = [];
                for(var i = 0;i < rainFallData.length; i++){
                    geoCoordMap[rainFallData[i].cityName.replace(/\s*/g,'')] = [rainFallData[i].latitude,rainFallData[i].longitude];  
                    data.push({
                        name:rainFallData[i].cityName.replace(/\s*/g,''),
                        value:rainFallData[i].rainfallYear
                    });
                }
        ...
        this.$axios({
            ...
            if(res.status == 200){
                ...
                for(var i = 0;i < lineData.length;i++){
                    ...
                    yAxisData.push(lineData[i].rainfallMonth)
                }
                var option = {
                    color:'	#00FA9A',  //折线图的颜色
                    tooltip: {
