...
Demo.prototype = {
    ...
    *******************************/
	...
	initDrawStruct : function (sentObj) {
		...
		for (i = 0; i < sentObj.length; i++) {
            ...
            // this.processSdpGraphNode(i, sentObj[i]["sem"], this.drawStruct); // todo
		}
    ...
    *******************************/
	...
	drawNerNode : function (nerNode, drawStruct, cxt) {
        ...
        heightObj ,
		paintHeight,
		// confSet = drawStruct.NER_DISCONF, //todo
		fontSize,
		fontBaselineHeight,
		fontX;
		//draw bg
		cxt.beginPath();
		// bgColor = confSet[nerNode.neName] == undefined ? "#0099cc" : confSet[nerNode.neName].bgcolor; // todo
	...
	update : function (disableAttr) {
		...
		this.setOffset( 0 , 20) ; // todo 画布位置（宽，高）
        this.move(0, 0 );
	},
	paint : function () {
		this.update();
    ...
    console.log(this.imageData) // todo，让canvas 自动适配内部imageData的宽高
	...
	addaptWidth : function () {
		...
		this.canvas.width = 550; //默认canvas宽度 TODO
